...
widgEditor.prototype.cleanPaste = function () {
  if (
    widgAutoClean ||
    confirm(
      "Do you wish to clean the HTML source of the content you just pasted?"
    )
  ) {
    var matchedHead = "";
    var matchedTail = "";
    var newContent = this.theIframe.contentWindow.document.getElementsByTagName(
      "body"
    )[0].innerHTML;
    ...
    for (
      newContentStart = 0;
      newContent.charAt(newContentStart) ==
    ...
    for (
      newContentFinish = 0;
      newContent.charAt(newContentFinish) ==
    ...
    newContent = newContent.reverse();
    newSnippet = newContent.substring(
      newContentStart,
    ...
    newSnippet = newSnippet.replace(/<[^>]*>/g, function (match) {
      match = match.replace(/ ([^=]+)="[^"]*"/g, function (
        match2,
        attributeName
      ) {
        if (
...
widgEditor.prototype.cleanSource = function () {
  ...
  if (this.wysiwyg) {
    theHTML = this.theIframe.contentWindow.document.getElementsByTagName(
      "body"
    )[0].innerHTML;
  } else {
  ...
  if (this.wysiwyg) {
    this.theIframe.contentWindow.document.getElementsByTagName(
      "body"
    )[0].innerHTML = theHTML;
  } else {
...
widgEditor.prototype.convertSPANs = function (theSwitch) {
  if (theSwitch) {
    ...
    var theSPANs = this.theIframe.contentWindow.document.getElementsByTagName(
      "span"
    );
    ...
    while (theSPANs.length > 0) {
      ...
      switch (theSPANs[0].getAttribute("style")) {
        case "font-weight: bold;":
          theReplacementElement = this.theIframe.contentWindow.document.createElement(
            "strong"
          );
        ...
        case "font-weight: bold; font-style: italic;":
          theParentElement = this.theIframe.contentWindow.document.createElement(
            "em"
          );
          theReplacementElement = this.theIframe.contentWindow.document.createElement(
            "strong"
      ...
      theSPANs = this.theIframe.contentWindow.document.getElementsByTagName(
        "span"
      );
    }
  } else {
    /* Replace em and strong tags with styled spans */
    var theEMs = this.theIframe.contentWindow.document.getElementsByTagName(
      "em"
    );
    ...
    var theSTRONGs = this.theIframe.contentWindow.document.getElementsByTagName(
      "strong"
    );
    ...
    while (theSTRONGs.length > 0) {
      ...
      theSTRONGs = this.theIframe.contentWindow.document.getElementsByTagName(
        "strong"
      );
    }
  }
...
widgEditor.prototype.detectPaste = function (e) {
  ...
  if (theEvent.ctrlKey && theEvent.keyCode == 86 && this.wysiwyg) {
    ...
    this.pasteCache = this.theIframe.contentWindow.document.getElementsByTagName(
      "body"
    )[0].innerHTML;
...
widgEditor.prototype.initEdit = function () {
  ...
  if (typeof document.addEventListener == "function") {
    this.theIframe.contentWindow.document.addEventListener(
      "mouseup",
      function () {
  ...
  /* IE event capturing */
    ...
    this.theIframe.contentWindow.document.attachEvent(
      "onkeydown",
      function (e) {
        self.detectPaste(e);
        return true;
...
widgEditor.prototype.insertNewParagraph = function (
  elementArray,
  succeedingElement
) {
  var theBody = this.theIframe.contentWindow.document.getElementsByTagName(
    "body"
  )[0];
...
widgEditor.prototype.paragraphise = function () {
  if (widgInsertParagraphs && this.wysiwyg) {
    ...
    for (var i = 0; i < theBody.childNodes.length; i++) {
      if (
        theBody.childNodes[i].nodeName.toLowerCase() == "#text" &&
...
widgEditor.prototype.refreshDisplay = function () {
  if (this.wysiwyg) {
    this.theIframe.contentWindow.document.getElementsByTagName(
      "body"
    )[0].innerHTML = this.theInput.value;
  } else {
...
widgEditor.prototype.writeDocument = function (documentContent) {
  ...
  if (typeof document.all == "undefined") {
    var stylesheet = this.theIframe.contentWindow.document.createElement(
      "link"
    );
    stylesheet.setAttribute("rel", "stylesheet");
    stylesheet.setAttribute("type", "text/css");
    stylesheet.setAttribute("href", widgStylesheet);
    this.theIframe.contentWindow.document
...
function widgToolbar(theEditor) {
  ...
  for (var i = 0; i < widgToolbarItems.length; i++) {
    switch (widgToolbarItems[i]) {
      ...
      case "unorderedlist":
        this.addButton(
          this.theList.id + "ButtonUnordered",
          "widgButtonUnordered",
          "Unordered List",
          "insertunorderedlist"
        );
...
widgToolbar.prototype.addSelect = function (
  theID,
  theClass,
  theContentArray,
  theAction
...
widgToolbar.prototype.disable = function () {
  ...
  for (var i = 0; i < this.theList.childNodes.length; i++) {
    ...
    if (
      theChild.nodeName.toLowerCase() == "li" &&
      theChild.className == "widgEditSelect"
    ) {
...
widgToolbar.prototype.setState = function (theState, theStatus) {
  if (theState != "SelectBlock") {
    var theButton = document.getElementById(
      this.theList.id + "Button" + theState
    );
...
function widgToolbarAction() {
  ...
  switch (this.action) {
    case "formatblock":
      theIframe.contentWindow.document.execCommand(
        this.action,
        false,
        this.value
      );
    ...
    case "link":
      if (this.parentNode.className.classExists("on")) {
        ...
        if (theIframe.contentWindow.document.selection) {
          theSelection = theIframe.contentWindow.document.selection.createRange()
            .text;
    ...
    case "image":
      ...
      if (theImage != null && theImage != "") {
        ...
        /* Mozilla selections */
          ...
          var theImageNode = theIframe.contentWindow.document.createElement(
            "img"
          );
    ...
    default:
      ...
      if (
        theIframe.contentWindow.document.queryCommandState(
          this.action,
          false,
          null
        )
    ...
    switch (theParentNode.nodeName.toLowerCase()) {
      ...
      case "span":
        if (theParentNode.getAttribute("style") == "font-weight: bold;") {
          ...
          theParentNode.getAttribute("style") == "font-style: italic;"
        ) {
          theWidgEditor.theToolbar.setState("Italic", "on");
        } else if (
          theParentNode.getAttribute("style") ==
      ...
      default:
        theWidgEditor.theToolbar.setState(
          "SelectBlock",
